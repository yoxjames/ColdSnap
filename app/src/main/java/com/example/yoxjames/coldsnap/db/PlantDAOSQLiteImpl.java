...
public class PlantDAOSQLiteImpl implements PlantDAO
{
    ...
    private final List<Plant> cachedPlants;
    ...
    public PlantDAOSQLiteImpl(Provider<Lazy<ContentValues>> contentValuesProvider, Provider<Lazy<PlantCursorWrapper.Factory>> plantCursorWrapperFactory)
    {
        ...
        this.cachedPlants = new ArrayList<>();
    }

    @Override
    public List<Plant> getPlants(SQLiteDatabase database)
    {
        ...
        try
        {
            ...
            {
                cachedPlants.add(cursor.getPlant());
                cursor.moveToNext();
            }
        }
        finally
        ...
        return new ArrayList<>(cachedPlants);
    }


    @Override
    public Plant getPlant(SQLiteDatabase database, UUID plantUUID)
    {
        if (cachedPlants.size() == 0)
            getPlants(database);

        for (Plant plant : cachedPlants)
        {
            if (plant.getUuid().equals(plantUUID))
                return plant;
        }

        throw new IllegalArgumentException("UUID: " + plantUUID + " was not found");
    }

    @Override
    public void addPlant(SQLiteDatabase database, Plant plant)
    {
    ...
    @Override
    public void deletePlant(SQLiteDatabase database, UUID plantUUID)
    {
    ...
    @Override
    public void updatePlant(SQLiteDatabase database, UUID plantUUID, Plant newPlant)
    {
        final String plantUUIDstring = plantUUID.toString();
        final ContentValues values = contentValuesProvider.get().get();
